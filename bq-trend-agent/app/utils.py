...
def get_latest_refresh_date(
  ...
  Args:
      table_name (str): The full ID of the BigQuery table to query, in the
        format 'project.dataset.table'. Defaults to the Google Trends
        international top terms table.
  ...
  try:
    ...
    print(
        f"❌ Table not found: The table '{table_name}' does not exist or you"
  ...
  try:
    ...
    if not project_id:
      raise ValueError(
          ...
          " environment variable or login with `gcloud auth application-default"
          " login`."
      )
